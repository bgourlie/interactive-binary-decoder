...
const FloatView = (props: FloatProps) => {
  ...
  const exponentBits = props.value.exponent_bits.map(
    (bit: boolean, i: number) => <div key={i}>{bit ? "1" : "0"}</div>
  );

  const mantissaBits = props.value.mantissa_bits.map(
    (bit: boolean, i: number) => <div key={i}>{bit ? "1" : "0"}</div>
  );
  ...
  const showActualFloatString =
    normalizedInputString && actualFloatString !== normalizedInputString;
  ...
  return (
    <div>
      <div className="inputRow">
        <input
          className={`float-input ${inputIsNaN ? "is-nan" : ""}`}
          type="text"
          placeholder="Enter a number"
          onChange={floatInputChangedHandler}
        />
        <div className={`decoded-value ${showActualFloatString ? "show" : ""}`}>
          {actualFloatString}
        </div>
      </div>
...
export function update(float: Float) {
  ReactDOM.render(
    <FloatView value={float} />,
    document.getElementById("float-view")
  );
}
